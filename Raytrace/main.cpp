...
vec3f trace(const Ray &ray, std::vector<Sphere> &spheres, const random_arr &random,int depth,const int &MAX_DEPTH) {
	...
	if ((sphere->transparency_>0 || sphere->reflection_>0) && (depth<MAX_DEPTH)) {
		...
		vec3f reflection = trace(Ray(hitinfo.phit_ + hitinfo.normal_*bias, reflec_dir), spheres, random, depth + 1, MAX_DEPTH);
		vec3f refraction;
		vec3f refrac_dir;
		vec3f k;
		float c;
		if (!inside) {
			...
			k = 1;
			refraction = trace(Ray(hitinfo.phit_ + hitinfo.normal_*bias, refrac_dir), spheres, random, depth + 1, MAX_DEPTH);
		}
		else {
		...
		for (unsigned int i = 0; i<spheres.size(); i++) {
			if (spheres[i].emissionColor_._x>0) {
				...
				for (int N = 0; N < random.n2; N++) {
					vec3f light_center = spheres[i].center_ + vec3f(random.s[N].x, random.s[N].y, 0);
					lightDirection = light_center - hitinfo.phit_;
			...
			surfaceColor = surfaceColor / float(random.n2);
		}
	}
	return surfaceColor + sphere->emissionColor_;
}
...
void demo_sphere_world(int w, int h, int MAX_DEPTH) {
	...
	spheres.push_back(Sphere(vec3f( -3.0, 0, -10), 3, vec3f(0.20, 0.20, 0.20), 0.4, 0.6,vec3f(0.7f,0.2f,0.2f)));
	spheres.push_back(Sphere(vec3f(0.0, -104, -20), 100, vec3f(0.6f, 0.6f, 0.5f), 1, 0,vec3f(0)));
	spheres.push_back(Sphere(vec3f(5.0, -1, -15), 2, vec3f(0.10, 0.46, 0.46), 0.6f, 0.4f, vec3f(0), 1.0f));
	spheres.push_back(Sphere(vec3f(5.0, 0, -25), 3, vec3f(0.3, 0.77, 0.4), 0.6f, 0.4f, vec3f(0), 1.33f));
	//spheres.push_back(Sphere(vec3f(-5.5, 0, -15), 3, vec3f(0.45, 0.3f, 0.2f), 0.1f, 0.9f, vec3f(0), 1.6f));
	// light
	spheres.push_back(Sphere(vec3f(0.0, 10, -30), 3, vec3f(0.6f, 0.20, 0.00), 0.4, 0.1, vec3f(0.6f, 0.20, 0.00)));
	spheres.push_back(Sphere(vec3f(0, 30, 0), 2, vec3f(0.6f, 0.00, 0.40), 0, 0, vec3f(0.6f, 0.00, 0.40)));
